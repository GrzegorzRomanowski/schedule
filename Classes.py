...
class LifeGuard:
    ...
    def addLifeGuardToEmployeeList(self):
        EmployeeList.append(self)       #.PESEL

    def __del__(self):
        EmployeeList.remove(self)       #.PESEL
...
class Pool:
    def __init__(self, name, amountOfLifeGuards):
        ...
        self.timeShifts = {"start1" : 7,
                        ...
                        "start2" : 15,
                        "end2" : 22}
        self.schifts = (list(range(1001, 1008))) * 2     #1032
        for c in self.schifts:
            i = self.schifts.index(c)
            if i < len(self.schifts) * 0.5:
                self.schifts[i] = c + 100       #str(c) +  "M"
            else:
                self.schifts[i] = c + 200       #str(c) +  "E"        
        a = 1
        schiftsAdded = []
        while a < self.amountOfLifeGuards:            
            for d in self.schifts:
                e = d + (1000 * a)
                schiftsAdded.append(e)
            a += 1
        for f in schiftsAdded:
            self.schifts.append(f)        
        fh = open(script_dir + "\\data\\" + str(self.name) + ".dat", "wb")
        pickle.dump(self.schifts, fh)
        fh.close
        self.schedule = {}
